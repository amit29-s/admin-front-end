...
const List = () => {
    ...
    const [initialRecords, setInitialRecords] = useState<iOrganization[]>([]);
    ...
    const fetchOrganizations = async () => {
        ...
        const organizations = await SendAsync<any>({
            url,
            method: 'GET',
        });
    ...
    useEffect(() => {
        setInitialRecords(sortBy(items, 'image'));
    }, [items]);
        ...
        setInitialRecords(() => {
            return items.filter((item) => {
                return (
                    ...
                    item.storeType.toLowerCase().includes(search.toLowerCase()) ||
    ...
    const deleteRow = async (id: number) => {
        if (window.confirm('Are you sure want to delete selected row ?')) {
            ...
            const url = `/organization/deleteOrganization/${id}`;
            try {
                await SendAsync<any>({
                    url,
                    method: 'DELETE',
    ...
    return (
        <div className="panel h-[90vh] border-white-light px-0 dark:border-[#1b2e4b]">
            <div className="invoice-table">
                <div className="mb-4.5 flex flex-col gap-5 px-5 md:flex-row md:items-center">
                    <div className="flex items-center gap-2">
                        <Link href="/apps/stores/add" className="btn btn-primary gap-2">
                        ...
                        columns={[
                            {
                                ...
                                sortable: true,
                                render: ({ image,_id }) => (
                                    <Link href={`/apps/stores/features/${_id}`}>
                                        <div className="flex h-24 w-40 items-center justify-center border border-neutral-200 p-2  ">
                                            <div className="flex h-20 items-center justify-center ">
                                                <img className="max-h-20" src={image} />
                                            </div>
                                        </div>
                                    </Link>
                                ),
                            },
                            {
                                accessor: 'storeName',
                                sortable: true,
                                render: ({ storeName, _id }) => (
                                    <Link href="/apps/stores/features">
